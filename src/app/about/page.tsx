...
import { FlipCard } from '@/component/Service';
...
const AboutUs = () => {
  ...
  const services = [
    { icon: MdCode, title: 'Custom Development', subtitle: 'Tailored software solutions', features: ["Web & Mobile Apps", "Enterprise Software", "APIs & Integrations"] },
    { icon: MdCloud, title: 'Cloud Solutions', subtitle: 'Scalable infrastructure', features: ["AWS, Azure, GCP", "Serverless Architecture", "DevOps & CI/CD"] },
    { icon: MdSecurity, title: 'Cybersecurity', subtitle: 'Advanced protection', features: ["Threat Analysis", "Penetration Testing", "Security Audits"] },
    { icon: MdSupport, title: '24/7 Support', subtitle: 'Round-the-clock assistance', features: ["Dedicated Support", "System Monitoring", "Incident Response"] }
  ];
        ...
        <div className={`mb-20 transform transition-all duration-1000 delay-800 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {team.map((member, index) => (
              <FlipCard key={index} title={member.name} subtitle={member.role} features={member.features} />
            ))}
          </div>
        </div>
